...
REPOS_DIR = "repos"
JSON_DIR = "jestable"
REVERSION_DATA_DIR = "commit_data"
        ...
        try:
            ...
            for i, commit in enumerate(pr_commits_chronological):
                ...
                if ss_files_in_this_commit and not first_ss_diff_text:
                    ...
                    first_commit_files = len(list(commit_diff.deltas))
                    first_commit_lines = sum(patch.line_stats[1] + patch.line_stats[2] for patch in commit_diff)
                    first_commit_snaps = len(ss_files_in_this_commit)
                    
                    expected_src_files = {get_source_file_from_snap(snap) for snap in ss_files_in_this_commit}
                    target_diffs = []
                    for patch in commit_diff:
                        file_path = patch.delta.new_file.path or patch.delta.old_file.path
                        if file_path in expected_src_files:
                            target_diffs.append(patch.data.decode('utf-8', 'replace'))
                    first_ss_diff_text = "\n".join(target_diffs)
            ...
            if pr_source_change_count > 0:
                ...
                if touched_ss_files:
                    ...
                    dataset_list.append({
                        ...
                        "first_commit_lines": first_commit_lines,
