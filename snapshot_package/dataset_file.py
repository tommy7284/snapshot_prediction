...
import snapshot_package.revert_Logic as revert_Logic

# --- Configuration ---
REPOS_DIR = "repos"
JSON_DIR = "jestable"
REVERSION_DATA_DIR = "file_data"
...
def get_merged_prs_data_via_graphql(repo_full_name: str, pr_numbers: list) -> dict:
    if not GITHUB_TOKEN:
        ...
        return {pr: {"body": ""} for pr in pr_numbers}
...
def process_unified_pr_data(repo, repo_path, repo_full_name):
    """Calculate and return both analysis statistics and dataset outputs simultaneously."""
    ...
    # === [Step 3] Concurrent detailed analysis & data extraction ===
    ...
    for pr_number, pr_info in merged_pr_info.items():
        ...
        try:
            ...
            # --- Variables for analysis and extraction ---
            snap_first_updates = {} 
            ...
            # Trace history from past to present (chronological)
            for i, commit in enumerate(pr_commits_chronological):
                ...
                for delta in commit_diff.deltas:
                    ...
                    if path and path.endswith(SNAPSHOT_EXTENSION) and revert_Logic.is_user_change(repo, commit, path):
                        is_valid_ss_update = True
                        
                        # The first entry here is guaranteed to be the 'initial update'
                        if path not in snap_first_updates:
                            src_path = get_source_file_from_snap(path)
                            
                            src_diff_text = ""
                            for patch in commit_diff:
            ...
            # ★ Append to analysis list and generate dataset
            if pr_source_change_count > 0:
                ...
                if snap_first_updates:
                    for snap_path, info in snap_first_updates.items():
                        ...
                        # Individually check if this specific .snap file was reverted
                        if revert_Logic.check_reversion(repo, snap_path, base_commit, last_non_merge, pr_commits):
                            ...
                            if base_oid is None and head_oid is None:
                                revert_type = "None=None"
                            else:
                                revert_type = "OID=OID"
    ...
    return pr_analysis_list, dataset_list, filtered_pr_count
...
# --- Functions for displaying previous analysis results ---
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(JSON_DIR):
        ...
        for data in jsons:
            ...
            output_path = os.path.join(REVERSION_DATA_DIR, f"{repo_dir_name}_reversions.csv")
            ...
            # Call unified process and receive both analysis list and CSV list simultaneously
            pr_analysis_list, dataset_list, scanned_count = process_unified_pr_data(repo, clone_dir, name_value)
            
            grand_total_scanned_prs += scanned_count
            
            if pr_analysis_list:
                grand_all_pr_analysis_results.extend(pr_analysis_list)
                grand_all_dataset_results.extend(dataset_list)
                write_to_csv(dataset_list, output_path)
            else:
                write_to_csv([], output_path)

    # Output statistics and save graph after processing all repositories
    if grand_total_scanned_prs > 0:
        calculate_and_print_final_stats(grand_all_pr_analysis_results, grand_total_scanned_prs)
        
        total_reverts = sum(1 for d in grand_all_dataset_results if d.get('reverted') == 1)
        none_none_count = sum(1 for d in grand_all_dataset_results if d.get('revert_type') == "None=None")
        oid_oid_count = sum(1 for d in grand_all_dataset_results if d.get('revert_type') == "OID=OID")
